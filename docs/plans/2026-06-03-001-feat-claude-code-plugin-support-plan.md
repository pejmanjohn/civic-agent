...
> **Post-implementation revision.** A follow-up requirement — show the bare `/civic-agent` in Claude Code's command picker, not the namespaced `/civic-agent:civic-agent` — was resolved by **naming the Claude Code plugin `civic-data`** (different from the `civic-agent` skill). A plugin skill is always invokable by its bare skill name; the picker shows the un-prefixed form only when the plugin name differs from the skill name (verified empirically, and matching how `compound-engineering`'s `/ce-plan` works). KTD1 (one shared plugin directory) **holds**: `plugins/civic-agent/` carries both `.codex-plugin/plugin.json` (name `civic-agent`) and the generated `.claude-plugin/plugin.json` (name `civic-data`) over one shared `skills/civic-agent/` tree. The Claude Code install id is therefore `civic-data@civic-agent`. (An interim attempt used a separate `plugins/civic-agent-cc/` root-`SKILL.md` "default skill"; that was based on a wrong premise — a plugin-root `SKILL.md` is still namespaced — and was reverted.) See `docs/architecture.md` for the current shape.
